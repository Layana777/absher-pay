...
const generateTopUpTransaction = (
  ...
  if (paymentMethod === "mada" || paymentMethod === "credit_card") {
    paymentDetails.lastFourDigits = Math.floor(1000 + Math.random() * 9000).toString();
...
const generateWithdrawalTransaction = (
  ...
  return {
    ...
    withdrawalDetails: {
      bankName,
      accountNumber: `SA${Math.floor(1000000000000000 + Math.random() * 9000000000000000)}`,
...
const generateTransactions = (
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (currentBalance < 0) {
      ...
      console.log(
        `   ⚠️  Skipped transaction (would cause negative balance: ${currentBalance.toFixed(2)} SAR)`
      );
      continue;
    }
